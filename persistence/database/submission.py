...
async def add(account_id: int, problem_id: int, task_id: Optional[int], language_id: int,
    ...
    async with SafeExecutor(
            ...
            sql="INSERT INTO submission"
                "            (account_id, problem_id, task_id, language_id,"
                "             content_file, content_length, submit_time)"
                "     VALUES (%(account_id)s, %(problem_id)s, %(task_id)s, %(language_id)s,"
                "             %(content_file)s, %(content_length)s, %(submit_time)s)"
                "  RETURNING id",
            account_id=account_id, problem_id=problem_id, task_id=task_id, language_id=language_id,
...
async def browse(account_id: int = None, problem_id: int = None, task_id: int = None, language_id: int = None) \
    ...
    async with SafeExecutor(
            ...
            sql=fr'SELECT id, account_id, problem_id, task_id, language_id,'
        ...
        return [do.Submission(id=id_, account_id=account_id, problem_id=problem_id, task_id=task_id,
                              language_id=language_id, content_file=content_file, content_length=content_length,
                              submit_time=submit_time)
                for id_, account_id, problem_id, task_id, language_id, content_file, content_length, submit_time
                in records]
...
async def read(submission_id: int) -> do.Submission:
    async with SafeExecutor(
            ...
            sql=fr'SELECT account_id, problem_id, task_id, language_id,'
                fr'       content_file, content_length, submit_time'
                fr'  FROM submission'
                fr' WHERE id = %(submission_id)s',
            submission_id=submission_id,
            fetch=1,
    ) as (account_id, problem_id, task_id, language_id, content_file, content_length, submit_time):
        return do.Submission(id=submission_id, account_id=account_id, problem_id=problem_id, task_id=task_id,
                             language_id=language_id, content_file=content_file, content_length=content_length,
                             submit_time=submit_time)
