...
async def read_team(team_id: int, request: auth.Request) -> do.Team:
    ...
    if team.is_hidden and not await rbac.validate(request.account.id, RoleType.manager, class_id=team.class_id):
        raise exc.NoPermission
...
async def edit_team(team_id: int, data: EditTeamInput, request: auth.Request) -> None:
    ...
    # 不是 CM 但卻碰了不該碰的東西
    if not is_class_manager and (data.is_hidden is not None or data.class_id is not None):
        raise exc.NoPermission
